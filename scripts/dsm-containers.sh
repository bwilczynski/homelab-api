...
# Login
SID=$(curl -s ${INSECURE_TLS:+-k} "${BASE}?api=SYNO.API.Auth&method=login&version=6&account=${DSM_USER}&passwd=${DSM_PASS}&format=sid" | jq -r '.data.sid')
...
# Logout
curl -s ${INSECURE_TLS:+-k} "${BASE}?api=SYNO.API.Auth&method=logout&version=6&_sid=${SID}" > /dev/null
echo ""
echo "=== Logged out ==="
